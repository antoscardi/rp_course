...
bool World::collides(const IntPoint& p, const int& radius) const {
  ...
  for (int r=-radius; r<=radius; ++r) {
    for (int c=-radius; c<=radius; ++c) {
      ...
      //cerr << "r: " << r << " c: " << c << " val: " << (int) at(p_test) << endl; 
      if (at(p_test)<127)
        return true;
    }
  }
...
void World::draw() {
  ...
  // angle += M_PI/180;
  // cv::line(_display_image, cv::Point(origin.y, origin.x), cv::Point(epi.y, epi.x), cv::Scalar(127,127,127), 1);
...
bool World::traverseBeam(IntPoint& endpoint,
