...
float VecF::operator*(const VecF& other) const {
  ...
  assert(other.dim==dim && "dim mismatch");
  for (int i=0; i<dim; ++i)
//acc += other.v[i]*acc; sbagluato visto che devi moltiplicare l elemento del vettore, 
//quindi usa v[i] che appartiene a ogni vettore della classe, invece è corretto:
  acc += v[i] * other.v[i];
  return acc;
}
