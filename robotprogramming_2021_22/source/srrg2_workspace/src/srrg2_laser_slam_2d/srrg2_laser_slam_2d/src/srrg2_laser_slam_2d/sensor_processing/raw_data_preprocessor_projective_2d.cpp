...
namespace srrg2_laser_slam_2d {
  ...
  void RawDataPreprocessorProjective2D::compute() {
    ...
    //TODO: fixme, adjust base link morte
    if (platform()) {
      Eigen::Isometry2f laser_on_robot;
      if (!platform()->getTransform(laser_on_robot, _scan_frame_id, "/base_link", _scan_time)) {
        _status = Error;
        return;
      }
      dest_matrix.transformInPlace<TRANSFORM_CLASS::Isometry>(laser_on_robot);
    }
    _meas->clear();
  ...
  void RawDataPreprocessorProjective2D::_processLaserMessage(LaserMessagePtr laser_message) {
    ...
    _scan_frame_id=laser_message->frame_id.value();
    _scan_time=laser_message->timestamp.value();
  }
