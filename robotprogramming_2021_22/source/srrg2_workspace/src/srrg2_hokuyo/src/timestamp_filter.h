...
public:
  ...
  inline void setMeasurement(const double& current_stamp_){
    if (!_calibration_rounds) {
      _status = Status::Ready;
      _filter_stamp=current_stamp_;
      return;
    }
  ...
  inline int calibrationRounds() const{
    return _calibration_rounds;
  }

  inline void setCalibrationRounds(int calibration_rounds_){
    _calibration_rounds = calibration_rounds_;
    reset();
  }
...
protected:
  ...
  inline void _initialize(const double& current_stamp_){
    ...
    if(_counter_init_stamps >= _calibration_rounds){
