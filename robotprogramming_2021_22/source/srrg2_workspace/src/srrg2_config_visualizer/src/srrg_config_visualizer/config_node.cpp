...
namespace srrg2_core {
  ...
  void ConfigNode::internals() {
    ...
    for (auto field : _configurable->properties()) {
      ...
      } else if (dynamic_cast<PropertyVector_<float>*>(field.second)) {
        prop_id_map.insert(std::make_pair(field.second, window->IDStack));
        if (ImGui::Button(name)) {
          ed::Suspend();
          ImGui::OpenPopup(popup_name);
    ...
    for (auto elem : prop_id_map) {
      ...
      if (auto p = dynamic_cast<PropertyVector_<int>*>(elem.first)) {
        ...
        ed::Resume();
      } else if (auto p = dynamic_cast<PropertyVector_<float>*>(elem.first)) {
        std::vector<float> values_float = p->value();
        uint64_t size                   = values_float.size();

        ed::Suspend();
        if (ImGui::BeginPopup(popup_name)) {
          ImGui::PushItemWidth(70);
          ImGui::InputScalar("vector size", ImGuiDataType_U64, &size, NULL, NULL, "%u");
          if (values_float.size() != size) {
            values_float.resize(size);
          }
          for (uint64_t i = 0; i < size; ++i) {
            std::string s = "val " + std::to_string(i);
            ImGui::DragScalar(s.c_str(), ImGuiDataType_Float, &values_float[i], 1);
          }
          if (!ImGui::IsPopupOpen(popup_name)) {
            p->setValue(values_float);
          }
          ImGui::PopItemWidth();
          ImGui::EndPopup();
        }
