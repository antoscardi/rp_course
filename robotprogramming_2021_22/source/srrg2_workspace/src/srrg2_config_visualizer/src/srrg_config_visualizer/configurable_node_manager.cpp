...
namespace srrg2_core {
  ...
  void ConfigurableNodeManager::deleteLink(ax::NodeEditor::LinkId& link_) {
    int idx=_findLink(link_);
    if (idx<0)
      return;
    NodeLinkPtr link      = _links[idx];
    ConfigNodePtr parent_node = link->parent();
    ConfigNodePtr child_node  = link->child;
    const std::string& param_name= link->paramName();
    if (! parent_node || ! child_node) {
      cerr << "bad, bad, bad, disconnected link" << endl;
      return;
    }

    // we erase from the parent vector the child
    auto prop_it              = parent_node->configurable()->properties().find(param_name);
    if (prop_it == parent_node->configurable()->properties().end()) {
      std::cerr << "class [" << parent_node->configurable()->className()
                << "] does not have field [" << param_name << "]" << std::endl;
    ...
    if (auto pcv = dynamic_cast<PropertyConfigurableVector*>(prop_it->second)) {
      for (size_t i=0; i<pcv->size(); ++i) {
        if (pcv->getSharedPtr(i)==child_node->configurable()) {
          pcv->eraseAt(i);
        }
      }
    } else if (auto pc = dynamic_cast<PropertyConfigurable*>(prop_it->second)) {
    ...
    ax::NodeEditor::DeleteLink(link_);
    _links.erase(_links.begin()+idx);
  }
  ...
  int ConfigurableNodeManager::_findLink(const ax::NodeEditor::LinkId& id) {
    int k=0;
    for (auto l: _links) {
      if (l->ID()==id)
        return k;
