...
namespace srrg2_core {
  ...
  public:
    ...
    void deleteLink(ax::NodeEditor::LinkId& link_);
  ...
  protected:
    ...
    int _findLink(const ax::NodeEditor::LinkId& id);
    ...
    inline void _releaseLinks(ConfigNodePtr node_) {
      std::set<NodeLinkPtr> this_links;
      for (auto& l: node_->inputLinks())
        this_links.insert(l);
      for (auto& l: node_->outputLinks())
        this_links.insert(l.second);
      for (auto& l: this_links) {
        auto l_to_del = std::find(_links.begin(), _links.end(), l);
      ...
      // cerr << "releasing input links" << endl;
      // for (auto it : i_links) {
      //   it->release();
      ...
      // cerr << "releasing output links" << endl;
      ...
      //   }
      // }
      cerr << "releasing connections" << endl;
