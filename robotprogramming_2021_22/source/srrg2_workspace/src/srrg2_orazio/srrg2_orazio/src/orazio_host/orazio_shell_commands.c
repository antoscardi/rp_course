...
int hardResetFn(char* r, char** args){
  return OrazioClient_hardReset(client, 0);
}
...
int sendFn(char* r, char** args){
  ...
  if (var->type==TypePacket){
    //printf("sending!!! %x %d %d\n",var->ptr,  ((PacketHeader*)var->ptr)->type, ((PacketHeader*)var->ptr)->size);
...
int paramFn(char* r, char* cmd_name, char** args, ParamAction action){
  ...
  if(! strcmp(var_name,"system_params")){
    ...
    query.param_type=ParamJointsSingle;
    query.index=1;
  } else if(! strcmp(var_name,"joint_params[2]")){
    query.param_type=ParamJointsSingle;
    query.index=2;
  } else if(! strcmp(var_name,"joint_params[3]")){
    query.param_type=ParamJointsSingle;
    query.index=3;
  ...
  } else if(! strcmp(var_name,"servo_params")){
    query.param_type=ParamServo;
  } else if(! strcmp(var_name,"imu_params")){
...
int loadConfigFn(char* r, char** args){
  ...
  while(!feof(f)){
    char var_name[1024];
    char var_value[1024];
    int res __attribute__((unused)) = fscanf(f,"%s %s\n", var_name, var_value);
    printf("[%s]=[%s]\n", var_name, var_value);
    VEntry* var=findVar(var_name);
    if(! var){
      r+=sprintf(r, "set: unknown variable [%s]\n", var_name);
...
Command commands[] = {
  ...
  },
  {
    ...
    .n_args = 0,
    .cmd_fn=hardResetFn,
    .help="usage: reset (requires board reboot)"
  },
  {
    .name = "fetch",
    .n_args = 1,
