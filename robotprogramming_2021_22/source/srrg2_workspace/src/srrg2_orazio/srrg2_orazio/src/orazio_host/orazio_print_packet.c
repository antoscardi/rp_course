...
int SystemParamPacket_print(char* f, SystemParamPacket* p){
  return sprintf(f, "{seq:%05d, protocol:%x, firmware:%x, period:%d, baud:%d, com_c:%d,  wd:%d, mot:%d }",
	  p->header.seq,
	  p->protocol_version,
	  p->firmware_version,
	  p->timer_period_ms,
...
int JointInfo_print(char* f, JointInfo* j){
  return sprintf(f, "{en:%d, m:%d, pos: %d, ms: %d, ds: %d, pwm: %d, curr:%d}",
                 j->enabled,
                 j->mode,
                 j->encoder_position,
                 j->encoder_speed,
...
int JointStatusPacket_print(char* f, JointStatusPacket* p){
  char* f_end=f;
  f_end+=sprintf(f_end, "{seq:%05d, j%d:", p->header.seq, p->header.index);
...
int SonarStatusPacket_print(char* f, SonarStatusPacket* p){
  return sprintf(f, "{seq:%05d, [%f %f %f %f %f %f %f %f]}",
                 p->header.seq,
                 p->ranges[0],
                 p->ranges[1],
                 p->ranges[2],
...
int ServoStatusPacket_print(char* f, ServoStatusPacket* p){
  return sprintf(f, "{seq:%05d, [%u %u %u]}",
                 p->header.seq,
                 p->servo_value[0],
                 p->servo_value[1],
                 p->servo_value[2]);
...
int ServoParamPacket_print(char* f, ServoParamPacket* p){
  return sprintf(f, "{seq:%05d, [%d %d %d]}",
                 p->header.seq,
                 (uint32_t)p->servo_pins[0],
                 (uint32_t)p->servo_pins[1],
                 (uint32_t)p->servo_pins[2]);
}
...
void Orazio_printPacketInit(void){
  ...
  _packet_print_ops[SERVO_STATUS_PACKET_ID]=(PrintPacketFn)ServoStatusPacket_print;
  _packet_print_ops[SERVO_PARAM_PACKET_ID]=(PrintPacketFn)ServoParamPacket_print;
