...
typedef struct {
  ...
  VEntry *joint_status_vars[NUM_JOINTS_MAX][NUM_VARIABLES];
  VEntry *joint_params_vars[NUM_JOINTS_MAX][NUM_VARIABLES];
  VEntry *joint_control_vars[NUM_JOINTS_MAX][NUM_VARIABLES];
  VEntry *drive_status_vars[NUM_VARIABLES];
  VEntry *drive_params_vars[NUM_VARIABLES];
  VEntry *drive_control_vars[NUM_VARIABLES];
  VEntry *servo_status_vars[NUM_VARIABLES];
  VEntry *servo_params_vars[NUM_VARIABLES];
  VEntry *servo_control_vars[NUM_VARIABLES];
  VEntry *imu_status_vars[NUM_VARIABLES];
  VEntry *imu_params_vars[NUM_VARIABLES];
  VEntry *imu_control_vars[NUM_VARIABLES];
...
void printVarInit(OrazioWSContext* ctx){
  ...
  printf("servo_status_vars: %d\n",getVarsByPrefix(ctx->servo_status_vars, "servo_status."));
  printf("servo_params_vars: %d\n",getVarsByPrefix(ctx->servo_params_vars, "servo_params."));
  printf("servo_control_vars: %d\n",getVarsByPrefix(ctx->servo_control_vars, "servo_control."));
  printf("imu_status_vars: %d\n",getVarsByPrefix(ctx->imu_status_vars, "imu_status."));
  printf("imu_params_vars: %d\n",getVarsByPrefix(ctx->imu_params_vars, "imu_params."));
  printf("imu_control_vars: %d\n",getVarsByPrefix(ctx->imu_control_vars, "imu_control."));
...
static int callback_http(struct lws *wsi,
  ...
  case LWS_CALLBACK_HTTP: 
    ...
    if(strstr(requested_uri,"system")){
      ...
      ctx->mode=Joint1;
    } else if(strstr(requested_uri,"joint2")){
      ctx->mode=Joint2;
    } else if(strstr(requested_uri,"joint3")){
      ctx->mode=Joint3;
    } else if(strstr(requested_uri,"drive")){
      ctx->mode=Drive;
    } else if(strstr(requested_uri,"servo")){
      ctx->mode=Servo;
    } else if(strstr(requested_uri,"imu")){
      ctx->mode=IMU;
...
void* _websocketFn(void* args){
  ...
  while (ctx->run) {
    ...
    char joint_status_string[NUM_JOINTS_MAX][BUF_SIZE];
    char system_params_string[BUF_SIZE];
    char joint_params_string[NUM_JOINTS_MAX][BUF_SIZE];
    char drive_status_string[BUF_SIZE];
    char drive_params_string[BUF_SIZE];
    char servo_status_string[BUF_SIZE];
    char servo_params_string[BUF_SIZE];
    char imu_status_string[BUF_SIZE];
    char imu_params_string[BUF_SIZE];
    ...
    printVarListHTML(servo_status_string,ctx->servo_status_vars);
    printVarListHTML(servo_params_string,ctx->servo_params_vars);
    printVarListHTML(imu_status_string,ctx->imu_status_vars);
    printVarListHTML(imu_params_string,ctx->imu_params_vars);
    ...
    case Joint2:
      mode_name="Joint2";
      status_string=joint_status_string[2];
      param_string=joint_params_string[2];
      break;
    case Joint3:
      mode_name="Joint3";
      status_string=joint_status_string[3];
      param_string=joint_params_string[3];
      break;
    case Drive:
      mode_name="Drive";
    ...
    case Servo:
      mode_name="Servo";
      status_string=servo_status_string;
      param_string=servo_params_string;
      break;
    case IMU:
      mode_name="IMU";
      status_string=imu_status_string;
      param_string=imu_params_string;
      break;
    case Sonar :
      mode_name="Sonar";
...
void OrazioWebsocketServer_genHtml(char* resource_path){
  ...
  sprintf(filename, "%s/%s",resource_path, "joint2_params.html");
  genVarInputHTML(&context, filename, context.joint_params_vars[2]);

  sprintf(filename, "%s/%s",resource_path, "joint2_control.html");
  genVarInputHTML(&context, filename, context.joint_control_vars[2]);

  sprintf(filename, "%s/%s",resource_path, "joint3_params.html");
  genVarInputHTML(&context, filename, context.joint_params_vars[3]);

  sprintf(filename, "%s/%s",resource_path, "joint3_control.html");
  genVarInputHTML(&context, filename, context.joint_control_vars[3]);
  ...
  sprintf(filename, "%s/%s",resource_path, "servo_params.html");
  genVarInputHTML(&context, filename, context.servo_params_vars);

  sprintf(filename, "%s/%s",resource_path, "servo_control.html");
  genVarInputHTML(&context, filename, context.servo_control_vars);

  sprintf(filename, "%s/%s",resource_path, "imu_params.html");
  genVarInputHTML(&context, filename, context.imu_params_vars);

  sprintf(filename, "%s/%s",resource_path, "imu_control.html");
  genVarInputHTML(&context, filename, context.imu_control_vars);
