...
typedef struct OrazioClient {
  ...
  ServoParamPacket servo_param;
  IMUStatusPacket imu_status;
  IMUParamPacket imu_param;
...
static PacketStatus _copyToBuffer(PacketHeader* p, void* args) {
  ...
  //printf("[%d]", p->type);
  if (p->type==MESSAGE_PACKET_ID) {
    printf("Message[%s]\n",((StringMessagePacket*)p)->message);
  }
  /*
  if (p->type==SONAR_STATUS_PACKET_ID) {
    printf("sonar[%d]\n",p->seq);
  }
  if (p->type==SONAR_PARAM_PACKET_ID) {
    printf("sonar_params[%d]\n",p->seq);
  }
  */
  return Success;
}
...
static PacketStatus _installPacketOp(OrazioClient* cl,
  ...
  if (dest_header) {
    dest_header->type=type;
    dest_header->size=size;
  }
  if (indexed) {
...
OrazioClient* OrazioClient_init(const char* device, uint32_t baudrate){
  ...
  _installPacketOp(cl, &cl->message, MESSAGE_PACKET_ID, PACKET_SIZE_ANY, 0);
  ...
  _installPacketOp(cl, &cl->servo_status, SERVO_STATUS_PACKET_ID, sizeof(cl->servo_status), 0);
  _installPacketOp(cl, &cl->servo_param, SERVO_PARAM_PACKET_ID, sizeof(cl->servo_param), 0);
  _installPacketOp(cl, &cl->imu_status, IMU_STATUS_PACKET_ID, sizeof(cl->imu_status), 0);
  _installPacketOp(cl, &cl->imu_param, IMU_PARAM_PACKET_ID, sizeof(cl->imu_param), 0);
  ...
  cl->packet_sizes[SERVO_PARAM_PACKET_ID]=sizeof(ServoParamPacket);
  cl->packet_sizes[SERVO_CONTROL_PACKET_ID]=sizeof(ServoControlPacket);
  cl->packet_sizes[IMU_PARAM_PACKET_ID]=sizeof(IMUParamPacket);
  cl->packet_sizes[IMU_CONTROL_PACKET_ID]=sizeof(IMUControlPacket);
  cl->packet_sizes[SONAR_PARAM_PACKET_ID]=sizeof(SonarParamPacket);
  cl->packet_sizes[HARD_RESET_COMMAND_PACKET_ID]=sizeof(HardResetCommandPacket);
  cl->packet_sizes[MESSAGE_PACKET_ID]=PACKET_SIZE_ANY;
...
PacketStatus OrazioClient_sendPacket(OrazioClient* cl, PacketHeader* p, int timeout){
  ...
  while(timeout>0){
    ...
    if (cl->response.p_type==awaited_type
      ...
      /*printf("got response %d, seq%d, result %d\n",
             (int) cl->response.p_type,
             (int) cl->response.p_seq,
             (int) cl->response.p_result);
      */
      send_result = Success;
      break;
    }
...
PacketStatus OrazioClient_readConfiguration(struct OrazioClient* cl, int timeout){
  ...
  do {
    ...
    ++trials;
  } while (status!=Success && trials<max_trials);
  if (status!=Success)
    return status;
  printf(" OK\n");
...
PacketStatus OrazioClient_get(struct OrazioClient* cl, PacketHeader* dest){
  ...
  pthread_mutex_lock(&cl->read_mutex);
  PacketStatus retval = GenericError;
  if (ops->on_receive_fn==_copyToBuffer) {
    PacketHeader* src=(PacketHeader*) ops->on_receive_args;
    int size=src->size;
    if (ops->size!=PACKET_SIZE_ANY && size!=ops->size) {
      printf("size mismatch\n");
      printf("type %d, desired_size: %d, got_size: %d\n", src->type, ops->size, size);
      retval=GenericError;
    } else {
      memcpy(dest, ops->on_receive_args, size);
      retval=Success;
    }
  }
  if (ops->on_receive_fn==_copyToIndexedBuffer) {
    // we retrieve the index from the destination packet
    PacketHeaderIndexed* p_idx=(PacketHeaderIndexed*) dest;
...
PacketStatus OrazioClient_hardReset(struct OrazioClient* cl, int timeout){
  HardResetCommandPacket cmd;
  cmd.header.type=HARD_RESET_COMMAND_PACKET_ID;
  cmd.header.size=sizeof(HardResetCommandPacket);
  printf("hard reset requeste\n");
  return OrazioClient_sendPacket(cl,(PacketHeader*)&cmd,10);
}
