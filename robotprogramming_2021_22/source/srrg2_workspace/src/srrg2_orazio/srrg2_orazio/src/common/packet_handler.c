...
PacketStatus _rxSize(PacketHandler* h, uint8_t c) {
  if (h->rx_current_op->size!=PACKET_SIZE_ANY
      && h->rx_current_op->size!=c) {
    h->rxFn=_rxAA;
...
static inline void _putTxByte(PacketHandler* h, uint8_t c){
  if (h->tx_size==PACKET_BUFFER_SIZE_MAX)
    return;
  h->tx_buffer[h->tx_end]=c;
  BUFFER_PUT(h->tx, PACKET_BUFFER_SIZE_MAX);
}
