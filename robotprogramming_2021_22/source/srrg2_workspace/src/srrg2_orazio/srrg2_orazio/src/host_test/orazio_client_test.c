...
static ServoControlPacket servo_control={
...
void* keyThread(void* arg){
  ...
  while(mode!=Stop) {
    ...
    case KeyArrowUp:
      switch (mode) {
      case Joints:
        ++joint_control[current_joint].control.speed;
        joint_control[current_joint].header.seq=1;
        break;
      case Drive:
        ...
        break;
      case Servo:
        ++servo_control.servo_value[current_servo];
        servo_control.header.seq=1;
        break;
      default:;
      }
      break;
    ...
    case KeyArrowRight:
      ...
      case Joints:
        ++current_joint;
        if (current_joint>=NUM_JOINTS_MAX)
          current_joint=0;
        break;
      case Drive:
    ...
    case KeyArrowLeft:
      ...
      case Joints:
        --current_joint;
        if (current_joint<0)
          current_joint=NUM_JOINTS_MAX-1;
        break;
      case Drive:
        drive_control.rotational_velocity+=0.1;
        drive_control.header.seq=1;
...
void toggleMode(Mode previous_mode, int previous_joint_index) {
  ...
  case Drive:
    ...
    break;
  case Ranges:
    params=(ParamPacketHeader*)&sonar_params;
    break;
  case Servo:
    params=(ParamPacketHeader*)&servo_params;
    break;
  case IMU:
    params=(ParamPacketHeader*)&imu_params;
    break;
  case Joints:
...
int main(int argc, char** argv) {
  ...
  while(mode!=Stop){
    ...
    if(servo_control.header.seq) {
      int result = OrazioClient_sendPacket(client, (PacketHeader*)&servo_control, 0);
      if (result)
        printf("send error\n");
      servo_control.header.seq=0;
    }
    ...
    case Ranges:
      ...
      break;
    case Servo:
      OrazioClient_get(client, (PacketHeader*)&servo_status);
      Orazio_printPacket(output_buffer,(PacketHeader*)&servo_status);
      printf("\r\033[2K%s",output_buffer);
      break;
    case IMU:
      OrazioClient_get(client, (PacketHeader*)&imu_status);
      Orazio_printPacket(output_buffer,(PacketHeader*)&imu_status);
      printf("\r\033[2K%s",output_buffer);
      break;
    case Drive:
